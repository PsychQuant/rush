...
Plugin wrapper for the `rush` MCP server (Swift, source at [PsychQuant/rush](https://github.com/PsychQuant/rush)).
...
|-------|----------------|
| `/rush:setup-tdx` | First-time TDX credential setup, or when SessionStart banner shows "⚠ TDX credentials missing" |
| `/rush:today-rail` | Quick O/D rail timetable lookups (台北→左營, 下一班高鐵, 末班自強號) |
| `/rush:nearby-bike` | YouBike geographic search (附近 YouBike, 哪裡借／還車) |
...
These come from the underlying Swift binary at `PsychQuant/rush`. Worth knowing because they shape tool behavior:
...
The binary reads credentials from macOS keychain service `che-transport-tdx`. SessionStart hook surfaces credential presence + binary version. If credentials missing → invoke `/rush:setup-tdx`.
...
| `binaryVersion` | The **Rush binary** release tag | `bin/rush-wrapper.sh` for auto-download |
...
1. **Source repo** `PsychQuant/rush`
   - Bump `Sources/Rush/Version.swift` (`AppVersion.version = "X.Y.Z"`)
   ...
   - `gh release create vX.Y.Z mcpb/server/Rush mcpb/server/Rush.sha256 mcpb/rush-X.Y.Z.mcpb mcpb/rush-X.Y.Z.mcpb.sha256 --repo PsychQuant/rush --title "vX.Y.Z — …" --notes-file <notes>`
   - **Critical**: include the **raw `Rush` binary** as a release asset, not just the `.mcpb`. The wrapper greps `browser_download_url` looking for `/Rush"` — without the raw binary asset, auto-download fails (the .mcpb is a Claude Desktop bundle, not what the wrapper consumes)

2. **Marketplace repo** `PsychQuant/psychquant-claude-plugins`
   - Bump **both** `version` and `binaryVersion` in `plugins/rush/.claude-plugin/plugin.json` to `X.Y.Z` (a binary release moves both in lockstep)
   ...
   - The fastest path: `/plugin-tools:plugin-update rush` — it bumps + commits + pushes + runs `claude plugin marketplace update` + `claude plugin update` automatically

3. **End-user side** (automatic, no action needed)
   - Wrapper sees `plugin.json.binaryVersion` ≠ `~/bin/.Rush.version` sidecar
...
- Source repo: <https://github.com/PsychQuant/rush>
- TDX register: <https://tdx.transportdata.tw/register>
- Design spec (in source repo): `docs/superpowers/specs/2026-05-20-rush-design.md`
